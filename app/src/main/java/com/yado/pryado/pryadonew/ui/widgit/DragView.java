    ...
    private void initDragView() {
        ...
        if (null != windowManager) {
            ...
            if (mScreenWidth > mScreenHeight) {
                mBuilder.size = mScreenHeight / 7;
            } else {
                mBuilder.size = mScreenWidth / 7;
            }
        }
        ...
        FrameLayout.LayoutParams layoutParams = createLayoutParams(mScreenWidth - mBuilder.size, mScreenHeight * 4 / 5, 0, 0);
        rootLayout = (FrameLayout) getActivity().getWindow().getDecorView();
    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (upListener != null) {
                    upListener.gestureUp();
                }
                float endX = event.getRawX();
                float endY = event.getRawY();
                if (Math.abs(endX - mStartX) > 7 || Math.abs(endY - mStartY) > 7) {
                ...
                Log.e("getDragView", getDragView().getX() + " 111  " + getDragView().getY());
                break;
        }
        return mTouchResult;
    }
