package com.yado.pryado.pryadonew.zxing.view;
...
public class FinderView extends View {
    ...
    public FinderView(Context context, AttributeSet attrs) {
        ...
        text1 = array.getString(R.styleable.ViewfinderView_text1);
        text2 = array.getString(R.styleable.ViewfinderView_text2);
        text3 = array.getString(R.styleable.ViewfinderView_text3);
        labelTextSize = array.getFloat(R.styleable.ViewfinderView_label_text_size, 44f);
    ...
    public void onDraw(Canvas canvas) {
        ...
        // Rect(162, 528 - 918, 1284)
        int viewHeight = getMeasuredHeight();
        int viewWidth = getMeasuredWidth();
        int scanHeight = (int) (0.25f * viewWidth);
        int centerX = viewWidth / 2;
        int centerY = viewHeight / 2;
//        KLog.e(this.getLeft()+":"+this.getTop());
//        KLog.e(viewHeight+":"+viewWidth);
//        KLog.e(screenWidth +":"+ screenHeight);
        frame.top = centerY - scanHeight;
        frame.left = centerX - scanHeight;
        frame.right = centerX + scanHeight;
        frame.bottom = centerY + scanHeight;
//        KLog.e(frame.toString());
    ...
    private void drawTextInfo(Canvas canvas, Rect frame) {
        ...
        textX = frame.left + frame.width() / 2;
        textY = frame.bottom + 2*CORNER_RECT_HEIGHT;
        if (checkedPos == 1) {
            paint.setColor(Color.YELLOW);
            canvas.drawText("· " + text1, textX, textY, paint);
            paint.setColor(labelTextColor);
            canvas.drawText(text2, textX + text1.length() * labelTextSize+CORNER_RECT_HEIGHT, textY, paint);
        } else {
            canvas.drawText(text1, textX - text1.length() * labelTextSize-CORNER_RECT_HEIGHT, textY, paint);
            paint.setColor(Color.YELLOW);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (checkedPos == 1) {
                    if (xx > textX + text1.length() * labelTextSize && xx < textX + text1.length() * labelTextSize + labelTextSize * 3 && yy > textY - labelTextSize * 2 && yy < textY + labelTextSize) {
                        checkedPos = 2;
                        show = false;
                        if (onTextClickListener != null) {
                            onTextClickListener.OnTextClick(checkedPos);
                        }
                    }
                } else {
                    if (xx > textX - (text1.length() + 2) * labelTextSize && xx < textX && yy > textY - labelTextSize * 2 && yy < textY + labelTextSize) {
                        checkedPos = 1;
                        show = true;
                        if (onTextClickListener != null) {
                            onTextClickListener.OnTextClick(checkedPos);
...
//    int middle = frame.screenHeight() / 2 + frame.top;
...
//      canvas.drawRect(frame.left, scannerStart, frame.right, scannerStart + SCANNER_LINE_HEIGHT, paint);
    ...
    public interface OnTextClickListener {
        void OnTextClick(int checkedPos);
    }
}
